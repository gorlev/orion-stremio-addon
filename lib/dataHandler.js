    ...
    if (listOpt === "both") {
        ...
        if (type === "movie") {
            ...
            for (let debridprovider of userConfiguration.debridservices) {
                movieDebridLinks = await getMovieCachedDebridLinks(keyuser,keyapp,idimdb,limitcount,videoquality, audiochannels,sortvalue, debridprovider, clientIp, audiolanguages,additionalParameters)
                allMovieDebridLinks = allMovieDebridLinks.concat(movieDebridLinks)
            }
            movieTorrentStreams = await getMovieTorrents(keyuser,keyapp,idimdb,limitcount,videoquality,audiochannels,sortvalue,audiolanguages,additionalParameters)
            ...
            for (let debridprovider of userConfiguration.debridservices) {
                seriesDebridLinks = await getSeriesCachedDebridLinks(keyuser,keyapp,idimdb,limitcount,videoquality,audiochannels,sortvalue,numberseason,numberepisode,debridprovider,clientIp,audiolanguages,additionalParameters)
                allSeriesDebridLinks = allSeriesDebridLinks.concat(seriesDebridLinks)
            }
            
            seriesTorrentStreams = await getSeriesTorrents(keyuser,keyapp,idimdb,limitcount,videoquality,audiochannels,sortvalue,numberseason,numberepisode,audiolanguages,additionalParameters)
